...
import { ArrowRight, ArrowLeft, Baby, Heart, Apple, Moon, Dumbbell, AlertCircle, Download, Ruler, Weight, Scale } from "lucide-react";
...
export default function WeekDetail() {
  ...
  const prev = week.weekNumber > 1 ? weeks[week.weekNumber - 2] : null;
  const next = week.weekNumber < 40 ? weeks[week.weekNumber] : null;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8" data-testid="page-week-detail">
      ...
      <section className="mb-10">
        <h2 className="text-xl font-bold text-foreground mb-4 flex items-center gap-2">
          <Apple className="w-5 h-5 text-primary" /> نصائح التغذية
        </h2>
        <ul className="space-y-2">
          {week.tips.nutrition.map((t, i) => (
      ...
      <div className="grid sm:grid-cols-2 gap-6 mb-10">
        <section>
          <h2 className="text-lg font-bold text-foreground mb-3 flex items-center gap-2">
            <Moon className="w-4 h-4 text-primary" /> نصائح النوم
          </h2>
          <ul className="space-y-2">
            {week.tips.sleep.map((t, i) => (
              <li key={i} className="flex items-start gap-3 text-sm text-foreground leading-relaxed">
                <span className="w-1.5 h-1.5 bg-indigo-400 rounded-full mt-2 shrink-0" /> {t}
              </li>
            ))}
          </ul>
        </section>
        <section>
          <h2 className="text-lg font-bold text-foreground mb-3 flex items-center gap-2">
            <Dumbbell className="w-4 h-4 text-primary" /> نصائح الرياضة
          </h2>
          <ul className="space-y-2">
            {week.tips.exercise.map((t, i) => (
              <li key={i} className="flex items-start gap-3 text-sm text-foreground leading-relaxed">
                <span className="w-1.5 h-1.5 bg-amber-400 rounded-full mt-2 shrink-0" /> {t}
              </li>
            ))}
          </ul>
        </section>
